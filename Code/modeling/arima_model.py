...
import pandas as pd
data=pd.df = pd.read_csv("Inputs/data/processed_data/cleaned_brent_prices_data.csv")
...
arima_rmse = np.sqrt(mean_squared_error(data['Price'][-10:], arima_forecast))
print(f'ARIMA RMSE: {arima_rmse}')

# Evaluate GARCH model
garch_forecast = garch_result.forecast(horizon=10)
garch_rmse = np.sqrt(mean_squared_error(data['Price'][-10:], garch_forecast.mean.values[-1]))
